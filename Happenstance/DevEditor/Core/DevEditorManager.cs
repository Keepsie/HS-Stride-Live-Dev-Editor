...
namespace Happenstance.SE.DevEditor.Core
{
    public class DevEditorManager : HSSyncScript
    {
        ...
        {
            _editorUIEntity = EntityFinder.FindChildByName(Entity, "DevEditorUI");
            ...
            _devCamera = EntityFinder.FindAllComponents<DevCamera>().FirstOrDefault();
            if (_editorUIEntity == null)
            {
        ...
        private Vector3 CalculateCameraRelativeMovement(DevCamera devCamera, float effectiveSpeed, bool rotateMode, bool scaleMode)
        {
            ...
            devCamera.Entity.Transform.UpdateWorldMatrix();
        ...
        private void ApplyRotationDelta(Vector3 delta)
        {
            ...
            Vector3 euler = GetEulerAnglesFromQuaternion(_selectedEntity.Transform.Rotation);
            ...
            _selectedEntity.Transform.Rotation = Quaternion.RotationYawPitchRoll(
                MathUtil.DegreesToRadians(euler.Y),
                MathUtil.DegreesToRadians(euler.X),
                MathUtil.DegreesToRadians(euler.Z));
        ...
        public void SetRotationX(float x)
        {
            ...
            var eulerAngles = GetEulerAnglesFromQuaternion(oldRot);
            eulerAngles.X = x;
            var newRot = Quaternion.RotationYawPitchRoll(
                MathUtil.DegreesToRadians(eulerAngles.Y),
                MathUtil.DegreesToRadians(eulerAngles.X),
                MathUtil.DegreesToRadians(eulerAngles.Z));
        ...
        public Vector3 GetEulerAnglesFromQuaternion(Quaternion quaternion)
        {
        ...
        public string FormatTransformForClipboard()
        {
            ...
            var rot = GetEulerAnglesFromQuaternion(_selectedEntity.Transform.Rotation);
